...
for k, sample in enumerate(samples):
    ...
    df = pd.read_csv("../res/count_bwa/" + sample + ".tsv", sep = " ", header = None)
    x, y = [], []
    _dict = {}

    for i in range(len(df.iloc[:,0])):
        _dict[str(int(df.iloc[i, 0]))] = df.iloc[i, 1]
    for i in list(range(int(df.iloc[-1, 0]))):
    ...
    plt.savefig('../res/plot_count_bwa/' + sample + '.png', dpi = 300)
    plt.close()
