...
workflow REPEAT_STAT_NANOPORE {
    ...
    main:
      ...
      // test_not_publish
      REPEAT_LENGTH_DISTRIBUTION_NANOPORE_UMI_CORRECT (
        REPEAT_LENGTH_DISTRIBUTION_NANOPORE.out.repeat_length_per_read_merge,
        params.umi_correction_method,
        params.umi_cutoffs)
      // 4_repeat_statistics/4a_repeat_length_distribution/repeat_length_count_nanopore_umi_x.csv|html
      STAT_REPEAT_LENGTH_DISTRIBUTION_NANOPORE_UMI_CORRECT (REPEAT_LENGTH_DISTRIBUTION_NANOPORE_UMI_CORRECT.out.repeat_length_count_default_umi_correct.collect(),
      params.umi_cutoffs)
      ...
      REPEAT_LENGTH_FRACTION (
        ...
        STAT_REPEAT_LENGTH_DISTRIBUTION_NANOPORE.out.csv,
        // master table for umi_x:
        STAT_REPEAT_LENGTH_DISTRIBUTION_NANOPORE_UMI_CORRECT.out.csv,
        params.allele_number,
        params.umi_cutoffs,
        )

      stat_table = STAT_REPEAT_LENGTH_DISTRIBUTION_NANOPORE.out.csv.mix(
        STAT_REPEAT_LENGTH_DISTRIBUTION_NANOPORE_UMI_CORRECT.out.csv
      ).collect()
